...
int cut_video_segment(const char* input_filename, const char* output_filename, double start_time, double duration) {
    AVFormatContext *input_ctx = NULL, *output_ctx = NULL;
    int64_t *start_pts_per_stream = NULL; // Array to store start PTS for each stream
    int ret = 0;
    ...
    start_pts_per_stream = calloc(input_ctx->nb_streams, sizeof(int64_t));
    if (!start_pts_per_stream) {
        ret = AVERROR(ENOMEM);
        goto cleanup;
    }
    
    for (int i = 0; i < input_ctx->nb_streams; i++) {
        start_pts_per_stream[i] = AV_NOPTS_VALUE;
    }
    ...
    // Seek to start time with better precision
    ...
    int64_t seek_time = seek_target_time - (AV_TIME_BASE / 2); // 0.5 seconds before
    if (seek_time < 0) seek_time = 0;
    
    ret = av_seek_frame(input_ctx, video_stream_index, 
                       av_rescale_q(seek_time, (AVRational){1, AV_TIME_BASE}, 
                                   video_stream_index >= 0 ? input_ctx->streams[video_stream_index]->time_base : (AVRational){1, AV_TIME_BASE}), 
    ...
    while (av_read_frame(input_ctx, packet) >= 0) {
        ...
        // Convert packet time to microseconds - handle case where pts might be invalid
        int64_t packet_time_us;
        if (packet->pts != AV_NOPTS_VALUE) {
            packet_time_us = av_rescale_q(packet->pts, input_stream->time_base, (AVRational){1, AV_TIME_BASE});
        } else if (packet->dts != AV_NOPTS_VALUE) {
            packet_time_us = av_rescale_q(packet->dts, input_stream->time_base, (AVRational){1, AV_TIME_BASE});
        } else {
        ...
        // Set start PTS for this stream if not already set
        // Use the first packet that's close to or after our target time
        if (start_pts_per_stream[packet->stream_index] == AV_NOPTS_VALUE) {
            // For the first part, use packets that are at or after the exact start time
            // For subsequent parts, be more lenient to avoid missing frames
            int64_t tolerance = (start_time > 0) ? (AV_TIME_BASE / 4) : 0; // 0.25 second tolerance for parts > 1
            
            if (packet_time_us >= (seek_target_time - tolerance)) {
                start_pts_per_stream[packet->stream_index] = (packet->pts != AV_NOPTS_VALUE) ? packet->pts : packet->dts;
            }
        }

        // Skip adjustment if we haven't set a start PTS for this stream yet
        if (start_pts_per_stream[packet->stream_index] == AV_NOPTS_VALUE) {
            av_packet_unref(packet);
            continue;
        }

        // Only skip packets that are significantly before our start time
        // This prevents missing frames due to keyframe seeking
        int64_t skip_threshold = seek_target_time - (AV_TIME_BASE / 2); // 0.5 seconds before
        if (start_time > 0 && packet_time_us < skip_threshold) {
            av_packet_unref(packet);
            continue;
        }

        // Calculate offset from the very first valid timestamp we want to include
        int64_t pts_offset = start_pts_per_stream[packet->stream_index];
        
        // Adjust timestamps relative to the start PTS for this specific stream
        if (packet->pts != AV_NOPTS_VALUE) {
...
void generate_output_filename(const char* input_filename, int part_number, char* output_filename, size_t max_len) {
    const char* dot = strrchr(input_filename, '.');
    if (dot) {
        size_t name_len = dot - input_filename;
        snprintf(output_filename, max_len, "%.*s_part%02d%s", (int)name_len, input_filename, part_number, dot);
    } else {
...
int split_video(const char* input_filename, double chunk_max_duration, double chunk_min_duration) {
    ...
    printf("Will create %d full chunks of %.1f hours each\n", full_chunks, chunk_max_duration / 3600.0);
    ...
    for (int i = 0; i < total_parts; i++) {
        ...
        if (i == total_parts - 1 && merge_last_chunk) {
            duration = last_chunk_duration; // chunk duration + remaining time
